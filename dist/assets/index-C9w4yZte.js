import{L,o as O}from"./vendor-B4umW4ZV.js";import{l as U}from"./workbox-pECmu2PR.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function t(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function i(s){if(s.ep)return;s.ep=!0;const a=t(s);fetch(s.href,a)}})();const _="https://story-api.dicoding.dev/v1",d=_,q=void 0;console.log(`Current API Base URL: ${d}`);async function h(n){const e=await n.json();if(!n.ok){const t=e.message||`Server error: ${n.status}`;throw new Error(t)}return e}async function z({name:n,email:e,password:t}){try{if(!n||typeof n!="string"||n.trim().length===0)throw new Error("Nama tidak boleh kosong");if(!e||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e))throw new Error("Email tidak valid");if(!t||t.length<8)throw new Error("Password harus minimal 8 karakter");console.log("Mengirim data registrasi:",{name:n,email:e,password:t});const i=await fetch(`${d}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:n,email:e,password:t})}),s=await h(i);return console.log("Respons dari server (register):",s),s}catch(i){throw console.error("Error registering:",i.message),new Error(i.message)}}async function N({email:n,password:e}){try{if(!n||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(n))throw new Error("Email tidak valid");if(!e||e.length<8)throw new Error("Password harus minimal 8 karakter");console.log("Mengirim data login:",{email:n,password:e});const t=await fetch(`${d}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:n,password:e})}),i=await h(t);return console.log("Respons dari server (login):",i),i}catch(t){throw console.error("Error logging in:",t.message),new Error(t.message)}}async function V({description:n,photo:e,lat:t,lon:i,token:s}){try{if(!n||n.trim().length===0)throw new Error("Deskripsi tidak boleh kosong");if(!e)throw new Error("Foto wajib diunggah");if(!s)throw new Error("Token tidak ditemukan, silakan login terlebih dahulu");const a=new FormData;a.append("description",n),a.append("photo",e),t&&i&&(a.append("lat",t),a.append("lon",i)),console.log("Mengirim data cerita:",{description:n,lat:t,lon:i,token:s});const o=await fetch(`${d}/stories`,{method:"POST",headers:{Authorization:`Bearer ${s}`},body:a}),l=await h(o);return console.log("Respons dari server (addStory):",l),l}catch(a){throw console.error("Error adding story:",a.message),new Error(a.message)}}async function W({description:n,photo:e,lat:t,lon:i}){try{if(!n||n.trim().length===0)throw new Error("Deskripsi tidak boleh kosong");if(!e)throw new Error("Foto wajib diunggah");const s=new FormData;s.append("description",n),s.append("photo",e),t&&i&&(s.append("lat",t),s.append("lon",i)),console.log("Mengirim data cerita guest:",{description:n,lat:t,lon:i});const a=await fetch(`${d}/stories/guest`,{method:"POST",body:s}),o=await h(a);return console.log("Respons dari server (addGuestStory):",o),o}catch(s){throw console.error("Error adding guest story:",s.message),new Error(s.message)}}async function j({token:n,page:e=1,size:t=10,location:i=0}){try{if(!n)throw new Error("Token tidak ditemukan, silakan login terlebih dahulu");const s=new URLSearchParams({page:e,size:t,location:i});console.log("Mengambil daftar cerita dengan params:",s.toString());const a=await fetch(`${d}/stories?${s}`,{headers:{Authorization:`Bearer ${n}`}}),o=await h(a);return console.log("Respons dari server (getStories):",o),o}catch(s){throw console.error("Error getting stories:",s.message),new Error(s.message)}}async function G(n,e){try{if(!n)throw new Error("ID cerita tidak boleh kosong");if(!e)throw new Error("Token tidak ditemukan, silakan login terlebih dahulu");console.log("Mengambil detail cerita dengan ID:",n);const t=await fetch(`${d}/stories/${n}`,{headers:{Authorization:`Bearer ${e}`}}),i=await h(t);return console.log("Respons dari server (getStoryDetail):",i),i}catch(t){throw console.error("Error getting story detail:",t.message),new Error(t.message)}}async function J(n,e){try{if(!n)throw new Error("Data subscription tidak boleh kosong");if(!e)throw new Error("Token tidak ditemukan, silakan login terlebih dahulu");const t={endpoint:n.endpoint,keys:{p256dh:n.keys.p256dh,auth:n.keys.auth}};console.log("Mengirim data subscription ke server:",t);const i=await fetch(`${d}/notifications/subscribe`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify(t)}),s=await h(i);return console.log("Respons dari server (subscribeNotifications):",s),s}catch(t){throw console.error("Error subscribing to notifications:",t.message),new Error(t.message)}}async function K(n,e){try{if(!n)throw new Error("Endpoint tidak boleh kosong");if(!e)throw new Error("Token tidak ditemukan, silakan login terlebih dahulu");console.log("Menghapus subscription dengan endpoint:",n);const t=await fetch(`${d}/notifications/subscribe`,{method:"DELETE",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify({endpoint:n})}),i=await h(t);return console.log("Respons dari server (unsubscribeFromNotifications):",i),i}catch(t){throw console.error("Error unsubscribing from notifications:",t.message),new Error(t.message)}}async function Y(n,e){try{if(!e)throw new Error("Token tidak ditemukan, silakan login terlebih dahulu");const t={title:"New Story Added!",body:`${n.name} just shared a new story: ${n.description.substring(0,50)}...`,data:{storyId:n.id,url:`/#/detail/${n.id}`}};console.log("Triggering story notification:",t);const i=await fetch(`${d}/notifications/trigger`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify(t)});if(i.ok){const s=await i.json();return console.log("Notification triggered successfully:",s),s}else return console.warn("Notification trigger endpoint not available"),null}catch(t){return console.error("Error triggering story notification:",t.message),null}}class M{async getStories({token:e,page:t=1,size:i=10,location:s=1}){const a=await j({token:e,page:t,size:i,location:s});if(a.error)throw new Error(a.message);return a.listStory}async getStoryDetail(e,t){const i=await G(e,t);if(i.error)throw new Error(i.message);return i.story}async addStory({description:e,photo:t,lat:i,lon:s,token:a}){const o=await V({description:e,photo:t,lat:i,lon:s,token:a});if(o.error)throw new Error(o.message);return o}async addGuestStory({description:e,photo:t,lat:i,lon:s}){const a=await W({description:e,photo:t,lat:i,lon:s});if(a.error)throw new Error(a.message);return a}}function f(n,e=-6.2,t=106.8,i=13){const s=L.map(n).setView([e,t],i);return L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'}).addTo(s),s}function b(n,e,t,i){const s=L.marker([e,t]).addTo(n);return i&&s.bindPopup(i).openPopup(),s}function Q(n,e){n.on("click",t=>{const{lat:i,lng:s}=t.latlng;e(i,s)})}class P{constructor(e){this.view=e,this.model=new M}async loadStories(){try{const e=localStorage.getItem("token");if(!e){this.view.showMessage("Silakan login terlebih dahulu!",()=>{window.location.hash="#/login"});return}const t=await this.getStories(e);if(t.error)throw new Error(t.message);this.view.displayStories(t)}catch(e){this.view.showMessage(`Gagal memuat cerita: ${e.message}`,()=>{this.loadStories()})}}async getStories(e){return await this.model.getStories({token:e,page:1,size:10,location:1})}initMap(e,t,i,s){if(t&&i){const a=f(e,t,i,13);b(a,t,i,s)}}}function c(n,e={}){const t=document.createElement(n);return Object.entries(e).forEach(([i,s])=>{t.setAttribute(i,s)}),t}const X="story-app-db",Z=1,g="stories";async function w(){return await O(X,Z,{upgrade(e){e.objectStoreNames.contains(g)||e.createObjectStore(g,{keyPath:"id"}).createIndex("createdAt","createdAt")}})}async function I(n){const t=(await w()).transaction(g,"readwrite");await Promise.all([...n.map(i=>t.store.put(i)),t.done])}async function ee(){return(await w()).getAllFromIndex(g,"createdAt")}async function te(n){await(await w()).delete(g,n)}class ie{constructor(){this.presenter=new P(this)}async init(){const e=c("div",{class:"home-container",role:"region","aria-label":"Daftar cerita"});e.innerHTML=`
    ...
    `,document.getElementById("content").innerHTML="",document.getElementById("content").appendChild(e),await this.presenter.loadStories()}displayStories(e){const t=document.getElementById("story-list");t.innerHTML="",e.forEach(i=>{const s=c("div",{class:"story-item",role:"article"});s.innerHTML=`
      ...
      `,document.getElementById("retry-btn").addEventListener("click",t)):alert(e)}}function x(n){localStorage.setItem("token",n)}function u(){return localStorage.getItem("token")}function se(){localStorage.removeItem("token")}class ne{constructor(){this.installPrompt=null}async init(){const e=u(),t=c("div",{class:"landing-container",role:"main"});t.innerHTML=`
    ...
    `).join(""))}showMessage(e){alert(e)}}class ae{constructor(){this.presenter=new P(this)}async init(){const e=c("div",{class:"stories-page-container",role:"region","aria-label":"Daftar semua cerita"});e.innerHTML=`
    ...
    `,document.getElementById("content").innerHTML="",document.getElementById("content").appendChild(e),this.setupEventListeners(),await this.presenter.loadStories()}setupEventListeners(){const e=document.getElementById("refresh-btn"),t=document.getElementById("sort-select");e.addEventListener("click",()=>{this.presenter.loadStories()}),t.addEventListener("change",i=>{this.sortStories(i.target.value)})}displayStories(e){const t=document.getElementById("stories-list");if(t.innerHTML="",e.length===0){t.innerHTML=`
      ...
      `,document.getElementById("retry-btn").addEventListener("click",t)}else alert(e)}}function oe(n){const e="=".repeat((4-n.length%4)%4),t=(n+e).replace(/-/g,"+").replace(/_/g,"/"),i=window.atob(t),s=new Uint8Array(i.length);for(let a=0;a<i.length;++a)s[a]=i.charCodeAt(a);return s}function B(){return"serviceWorker"in navigator&&"PushManager"in window}async function D(){return"Notification"in window?await Notification.requestPermission()!=="granted"?(console.warn("Izin notifikasi tidak diberikan oleh pengguna."),!1):!0:(console.warn("Browser ini tidak mendukung notifikasi."),!1)}async function F(){const n=B();if(!n)return{isSupported:!1,permission:"denied",isSubscribed:!1};const e=Notification.permission,i=await(await navigator.serviceWorker.ready).pushManager.getSubscription();return{isSupported:n,permission:e,isSubscribed:!!i}}async function H(n){if(!B())throw new Error("Push notification tidak didukung di browser ini.");if(Notification.permission==="denied")throw new Error("Izin notifikasi telah ditolak. Ubah di pengaturan browser.");const e=await navigator.serviceWorker.ready;let t=await e.pushManager.getSubscription();if(!t){if(!await D())throw new Error("Izin notifikasi tidak diberikan.");const s=oe(q);t=await e.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:s})}try{return await J(t.toJSON(),n),console.log("Berhasil berlangganan push notification."),t}catch(i){throw console.error("Gagal mendaftar ke server, membatalkan subscription...",i),await t.unsubscribe(),new Error(`Gagal mendaftarkan subscription ke server: ${i.message}`)}}async function re(n){if(!B())return console.warn("Push notification tidak didukung."),!1;const t=await(await navigator.serviceWorker.ready).pushManager.getSubscription();if(!t)return console.log("Tidak ada subscription aktif untuk dihentikan."),!0;if(!await t.unsubscribe())return console.error("Gagal berhenti berlangganan di sisi client."),!1;try{await K(t.endpoint,n),console.log("Berhasil berhenti berlangganan sepenuhnya.")}catch(s){console.warn("Berhasil berhenti di client, tapi gagal memberitahu server.",s)}return!0}async function ce(n,e,t={}){if(Notification.permission==="granted")try{await(await navigator.serviceWorker.ready).showNotification(n,{body:e,icon:"/icons/icon-192x192.png",badge:"/icons/icon-96x96.png",tag:"story-notification",vibrate:[200,100,200],data:t})}catch(i){console.error("Gagal menampilkan notifikasi:",i)}}class le{constructor(e){this.view=e,this.model=new M,this.coordinates={},this.currentMarker=null,this.map=null}initMap(e){return this.map=f(e),Q(this.map,(t,i)=>{this.setCoordinates(t,i),this.view.updateLocationDisplay(t,i)}),this.map}setCoordinates(e,t){this.coordinates={lat:e,lon:t},this.currentMarker&&this.map.removeLayer(this.currentMarker),this.currentMarker=b(this.map,e,t,"Selected location")}updateMapLocation(e,t){this.map&&(this.map.setView([e,t],15),this.setCoordinates(e,t))}getCoordinates(){return this.coordinates}async addStory(e,t,i,s){const a=u();if(!a){this.view.showMessage("Authentication error. Please login again.","error");return}try{this.view.showLoading(!0);const o=await this.model.addStory({description:e,photo:t,lat:i||this.coordinates.lat,lon:s||this.coordinates.lon,token:a});await ce("Story Shared Successfully!",`Your story "${e.substring(0,30)}..." has been shared.`),this.view.showMessage("Story shared successfully!","success"),window.location.hash="#/stories",this.triggerNotificationInBackground(a)}catch(o){this.view.showMessage(`Failed to share story: ${o.message}`,"error")}finally{this.view.showLoading(!1)}}async triggerNotificationInBackground(e){try{console.log("Attempting to trigger push notification to other users..."),await Y(e),console.log("Push notification triggered successfully.")}catch(t){console.warn("Could not trigger push notification:",t.message)}}}async function de(n){try{const e=await navigator.mediaDevices.getUserMedia({video:{facingMode:"environment"}});return n.srcObject=e,e}catch(e){if(e.name==="NotFoundError"||e.name==="DevicesNotFoundError"){console.warn("Kamera belakang tidak ditemukan, mencoba kamera depan.");try{const t=await navigator.mediaDevices.getUserMedia({video:!0});return n.srcObject=t,t}catch(t){throw console.error("Gagal mengakses kamera fallback:",t),t}}throw console.error("Error saat mengakses kamera:",e),e}}function ue(n,e){const t=e.getContext("2d");return e.width=n.videoWidth,e.height=n.videoHeight,t.drawImage(n,0,0,e.width,e.height),e.toDataURL("image/jpeg")}function he(n){n&&typeof n.getTracks=="function"&&(n.getTracks().forEach(e=>e.stop()),console.log("Aliran kamera berhasil dihentikan."))}function pe(n,e){const t=n.split(","),i=t[0].match(/:(.*?);/);if(!i)throw new Error("Data URL tidak valid");const s=i[1],a=atob(t[1]);let o=a.length;const l=new Uint8Array(o);for(;o--;)l[o]=a.charCodeAt(o);return new File([l],e,{type:s})}class me{constructor(){this.presenter=new le(this),this.stream=null,this.photo=null,this.userLocation=null}async init(){if(!u()){this.showMessage("Silakan login terlebih dahulu!"),window.location.hash="#/login";return}const t=c("div",{class:"add-story-container"});t.innerHTML=`
    ...
    `;const i=document.getElementById("content")||document.body;i.innerHTML="",i.appendChild(t),await this.initializeForm()}async initializeForm(){const e=document.getElementById("camera");try{this.stream=await de(e)}catch(t){this.showMessage(`Failed to access camera: ${t.message}. Please ensure camera is available and allow camera access in browser settings.`);return}this.presenter.initMap("map"),this.setupEventListeners()}setupEventListeners(){const e=document.getElementById("description"),t=document.getElementById("char-count"),i=document.getElementById("capture"),s=document.getElementById("retake"),a=document.getElementById("use-my-location"),o=document.getElementById("cancel-btn"),l=document.getElementById("add-story-form");e.addEventListener("input",k=>{const T=k.target.value.length;t.textContent=T,t.style.color=T>500?"#ef476f":"#6c757d"}),i.addEventListener("click",()=>{this.capturePhoto()}),s.addEventListener("click",()=>{this.retakePhoto()}),a.addEventListener("click",()=>{this.getCurrentLocation()}),o.addEventListener("click",()=>{confirm("Are you sure you want to cancel? Your story will be lost.")&&(this.destroy(),window.location.hash="#/stories")}),l.addEventListener("submit",async k=>{k.preventDefault(),await this.submitStory()})}capturePhoto(){const e=document.getElementById("camera"),t=document.getElementById("canvas"),i=document.getElementById("preview"),s=document.getElementById("capture"),a=document.getElementById("retake"),o=ue(e,t);this.photo=pe(o,"story.jpg"),i.src=o,i.style.display="block",e.style.display="none",s.style.display="none",a.style.display="inline-flex"}retakePhoto(){const e=document.getElementById("camera"),t=document.getElementById("preview"),i=document.getElementById("capture"),s=document.getElementById("retake");this.photo=null,t.style.display="none",e.style.display="block",i.style.display="inline-flex",s.style.display="none"}async getCurrentLocation(){const e=document.getElementById("use-my-location"),t=e.innerHTML;e.innerHTML='<span class="spinner-small"></span> Getting location...',e.disabled=!0;try{const i=await new Promise((o,l)=>{navigator.geolocation.getCurrentPosition(o,l,{enableHighAccuracy:!0,timeout:1e4,maximumAge:6e4})}),{latitude:s,longitude:a}=i.coords;this.updateLocationDisplay(s,a),this.presenter.setCoordinates(s,a),this.presenter.updateMapLocation(s,a)}catch(i){this.showMessage(`Failed to get location: ${i.message}`)}finally{e.innerHTML=t,e.disabled=!1}}updateLocationDisplay(e,t){const i=document.getElementById("location-info"),s=document.getElementById("latitude"),a=document.getElementById("longitude");i.innerHTML=`
      <span class="location-icon">📍</span>
      <span class="location-text">Location: ${e.toFixed(6)}, ${t.toFixed(6)}</span>
    `,i.classList.add("location-selected"),s.value=e.toFixed(6),a.value=t.toFixed(6)}async submitStory(){const e=document.getElementById("description").value.trim();if(!e){this.showMessage("Please enter a story description!");return}if(e.length>500){this.showMessage("Story description must be 500 characters or less!");return}if(!this.photo){this.showMessage("Please capture a photo first!");return}const{lat:t,lon:i}=this.presenter.getCoordinates();await this.presenter.addStory(e,this.photo,t,i)}showLoading(e){const t=document.querySelector('button[type="submit"]');if(t)if(e)t.innerHTML='<span class="spinner-small"></span> Sharing...',t.disabled=!0;else{const i='<span class="btn-icon">📝</span> Share Story';t.innerHTML=i,t.disabled=!1}}destroy(){he(this.stream)}showMessage(e,t="info"){console.log(`[${t.toUpperCase()}]: ${e}`),alert(e)}}class ge{constructor(e){this.view=e,this.model=new M}async getStoryDetail(e,t){try{return await this.model.getStoryDetail(e,t)}catch(i){throw new Error(`Gagal mengambil detail cerita: ${i.message}`)}}initMap(e,t,i,s){if(t&&i){const a=f(e,t,i,13);b(a,t,i,s)}}}class ye{constructor(){this.presenter=new ge(this)}async init({id:e}){const t=u();if(!t){window.location.hash="#/login";return}const i=c("div",{class:"detail-container"});i.innerHTML=`
    ...
    `,document.getElementById("retry-btn").addEventListener("click",()=>{window.location.reload()})}showMessage(e){const t=c("div",{class:"toast-notification"});t.textContent=e,document.body.appendChild(t),setTimeout(()=>{t.classList.add("show")},100),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>{document.body.removeChild(t)},300)},3e3)}}class ve{constructor(e){this.view=e}async login({email:e,password:t}){try{const i=await N({email:e,password:t});if(i.error)throw new Error(i.message);x(i.loginResult.token),localStorage.setItem("userData",JSON.stringify({name:i.loginResult.name||e.split("@")[0],email:e})),this.view.showMessage("Login berhasil!"),window.location.hash="#/"}catch(i){i.message==="User not found"?this.view.showMessage("Pengguna tidak ditemukan. Silakan registrasi terlebih dahulu."):this.view.showMessage(`Login gagal: ${i.message}`)}}}class fe{constructor(){this.presenter=new ve(this)}async init(){const e=c("div",{class:"auth-container"});e.innerHTML=`
      <h2>Login</h2>
      <form id="login-form">
        <div class="form-group">
    ...
    `,document.getElementById("content").innerHTML="",document.getElementById("content").appendChild(e),await this.loadFavorites()}async loadFavorites(){try{this.stories=await ee(),this.displayStories()}catch(e){this.showMessage(`Gagal memuat cerita favorit: ${e.message}`)}}displayStories(){const e=document.getElementById("favorites-list");if(this.stories.length===0){e.innerHTML=`
      ...
      `,e.appendChild(i),t.lat&&t.lon&&this.initMap(`map-${t.id}`,t.lat,t.lon,t.name)}),document.querySelectorAll(".remove-favorite-btn").forEach(t=>{t.addEventListener("click",async i=>{const s=i.target.dataset.storyId;await this.removeFavorite(s)})})}async removeFavorite(e){try{await te(e),await this.loadFavorites(),this.showMessage("Cerita berhasil dihapus dari favorit!")}catch(t){this.showMessage(`Gagal menghapus cerita: ${t.message}`)}}initMap(e,t,i,s){if(t&&i){const a=f(e,t,i,13);b(a,t,i,s)}}showMessage(e){alert(e)}}class Se{async init(){const e=c("div",{class:"about-container",role:"main"});e.innerHTML=`
  ...
  `,document.body.appendChild(e),document.getElementById("update-btn").addEventListener("click",()=>{n.messageSkipWaiting(),e.remove(),r.trackEvent("app_update_accepted")}),document.getElementById("dismiss-update").addEventListener("click",()=>{e.remove(),r.trackEvent("app_update_dismissed")})}function p(n,e="success"){const t=document.createElement("div");t.className=`toast-notification ${e}`,t.textContent=n,t.setAttribute("role","alert"),t.setAttribute("aria-live","polite"),document.body.appendChild(t),setTimeout(()=>{t.classList.add("show")},100),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>{document.body.contains(t)&&document.body.removeChild(t)},300)},3e3)}window.addEventListener("hashchange",()=>{setTimeout(E,100),r.trackPageView(window.location.hash)});window.addEventListener("storage",n=>{n.key==="token"&&(setTimeout(E,100),r.trackEvent("auth_state_changed",{source:"storage_event"}))});window.addEventListener("online",()=>{p("Connection restored","success"),r.trackEvent("connection_restored")});window.addEventListener("offline",()=>{p("You are offline. Some features may be limited.","warning"),r.trackEvent("connection_lost")});window.addEventListener("load",()=>{setTimeout(()=>{const n=performance.getEntriesByType("navigation")[0];n&&(r.trackPerformance("page_load_time",n.loadEventEnd-n.loadEventStart),r.trackPerformance("dom_content_loaded",n.domContentLoadedEventEnd-n.domContentLoadedEventStart))},0)});Pe();
//# sourceMappingURL=index-C9w4yZte.js.map
